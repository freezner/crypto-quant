...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(f"💰 {format_korean_money(int(initial_balance))}")
...
def show_daily_prices(exchange_name: str, symbol: str, period_days: int):
    ...
    try:
        ...
        display_df.columns = ["날짜", "시가", "고가", "저가", "종가", volume_col]
        ...
        display_df[volume_col] = display_df[volume_col].apply(lambda x: f"{x:,.2f}")
